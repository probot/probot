...
class Robot {
  ...
  async auth (id, log = this.log) {
    ...
    if (id) {
      const cacheString = `app:${id}:token`
      let token = await this.cache.get(cacheString)

      if (!token) {
        log.trace(`creating token for installation`)
        github.authenticate({type: 'integration', token: this.app()})
        const res = await github.apps.createInstallationToken({installation_id: id})
        token = res.data.token
        await this.cache.set(cacheString, token, 60 * 59)  // Cache for 1 minute less than GitHub expiry
      }

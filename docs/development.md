...
2. Start a [`Server`](https://probot.github.io/api/latest/modules/server.html) instance
...
The `server` instance gives you access to the express app instance (`server.expressApp`) as well as the [`Probot`](https://probot.github.io/api/latest/modules/probot.html) instance (`server.probotApp`).
...
If you don't use Probot's http handling in order to receive and verify events from GitHub via webhook requests, you can use the [`Probot`](https://probot.github.io/api/latest/modules/probot.html) class directly.

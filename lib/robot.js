...
// List of events causing GitHub App to be unable to authenticate itself (#382):
const breakingEvents = ['installation.created', 'installation.deleted']
...
class Robot {
   ...
   */
  on (webhook, callback) {
    if (webhook.constructor === Array) {
      webhook.forEach(e => this.on(e, callback))
      return
    }

    const [name, action] = webhook.split('.')

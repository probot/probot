...
behavior
  = on:on cond:if then:then semicolon {
    return {on: on, conditions: cond, then: then};
  }
  / on:on then:then semicolon { return {on: on, then: then};  }
...
expression = left:word ws operand:operand ws right:word {
  return {type:'expression', left: left, operand: operand, right: right};
}

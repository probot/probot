...
module.exports = (options = {}) => {
  ...
  webhook.on('error', async err => {
    ...
    const appDetails = await github.apps.get({})
    const appName = /\/(.(?!\/))+$/.exec(appDetails.data.html_url)[0]

    switch (err.message) {
      case 'X-Hub-Signature does not match blob signature':
        logger.error(`Go to https://github.com/settings/apps${appName} and verify that the Webhook secret matches the value of the WEBHOOK_SECRET environment variable.`)
        break
      case 'error:0906D06C:PEM routines:PEM_read_bio:no start line':
        logger.error(`Your private key (usually a .pem file) is not correct. Go to https://github.com/settings/apps${appName} and generate a new PEM file. If you're deploying to Now, visit https://probot.github.io/docs/deployment/#now.`)
        break
      default:
        logger.error(err)
    }

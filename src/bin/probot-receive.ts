...
async function main() {
  ...
  const githubToken = program.opts().token;

  if (!program.opts().event || !program.opts().payloadPath) {
    program.help();
  }

  const privateKey = getPrivateKey();
  if (!githubToken && (!program.opts().app || !privateKey)) {
    console.warn(
      "No token specified and no certificate found, which means you will not be able to do authenticated requests to GitHub",
    );
  }

  const payload = require(path.resolve(program.opts().payloadPath));
  const log = getLog({
    level: program.opts().logLevel,
    logFormat: program.opts().logFormat,
    logLevelInString: program.opts().logLevelInString,
    logMessageKey: program.opts().logMessageKey,
    sentryDsn: program.opts().sentryDsn,
  });

  const probot = new Probot({
    appId: program.opts().app,
  ...
  probot.log.debug("Receiving event", program.opts().event);
  probot
    .receive({ name: program.opts().event, payload, id: uuidv4() })
    .catch(() => {

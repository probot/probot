...
let IssuePlugin = superclass => class extends superclass {
  ...
  react(reaction) {
    this._setCommentData({reaction: reaction});
    return this;
  }
...
class IssueEvaluator extends base.Evaluator {
  evaluate(workflow, context) {
    ...
    if (workflow.issueActions.unassignees !== undefined) {
      let assignees = {assignees: workflow.issueActions.unassignees};
      promises.push(
    ...
    if (workflow.issueActions.unlabels !== undefined) {
      let labels = workflow.issueActions.unlabels;
      promises.push(
        labels.map(label => {
    ...
    if (workflow.issueActions.reaction !== undefined) {
      let reaction = workflow.issueActions.reaction;
      promises.push(

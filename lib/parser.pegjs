start = b:behavior* { return {behaviors: b}; }

behaviors = behavior*

behavior = ws on:on ws then:then semicolon ws { return {on: on, then: then}; }
...
events
  = event:event ws and ws events:events { return [event].concat(events); }

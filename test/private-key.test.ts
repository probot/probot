  ...
  describe("findPrivateKey(undefined)", () => {
    ...
    describe("when a PRIVATE_KEY_PATH env var is provided but file is not present at that path", () => {
      ...
      it("should throw an error", () => {
        expect(findPrivateKey).toThrow(
          `Private key does not exists at path: ${keyfilePath}. Please check to ensure that the PRIVATE_KEY_PATH is correct.`
        );
      });
    });
    ...
    describe("when no private key is provided", () => {
      beforeEach(() => {
        fs.readdirSync = jest.fn().mockReturnValue(["foo.txt", "foo.pem"]);
      });
      ...
      describe("and several key files are present", () => {
        beforeEach(() => {
          fs.readdirSync = jest
            .fn()
            .mockReturnValue(["foo.txt", "foo.pem", "bar.pem"]);
        });

        it("should throw an error", () => {
          expect(findPrivateKey).toThrow(
            /Found several private keys: foo.pem, bar.pem/i
          );
        });
      });

...
behavior
  = events:on cond:if then:then semicolon {
    return {type: 'behavior', events: events, conditions: cond, then: then};
  }
  / events:on then:then semicolon {
    return {type: 'behavior', events: events, then: then};
  }

...
class Robot {
   ...
   * });
   */
  async on (event, ...functions) {
    if (event.constructor === Array) {
      event.forEach(e => this.on(e, ...functions))
      return
    }
    ...
    return this.events.on(name, async event => {
      if (!action || action === event.payload.action) {
        ...
        try {
          ...
          let context = new Context(event, github, log)
          let callback
          if (functions.length === 1) {
            callback = functions[0]
          } else {
            callback = functions.splice(-1)[0]

            const next = async (err, ctx) => {
              if (err) {
                log.error({err, event})
                if (!this.catchErrors) {
                  throw err
            ...
            for (let i = 0; i < functions.length; i++) {
              ...
              if (flag) await func(context, next, done)
            }

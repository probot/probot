...
class Robot {
    ...
    return this.events.on(name, async event => {
      if (!action || action === event.payload.action) {
        ...
        try {
          ...
          const ql = await this.getGraphQLClient(event.payload.installation.id)
          const context = new Context(event, github, ql, log)
  ...
  /**
   * Authenticate and get a GitHub GraphQL client that can be used to make API calls.
   *
   * You'll probably want to use `context.ql` instead.
   *
   * **Note**: `robot.getGraphQLClient` is asynchronous, so it needs to be prefixed with a
   * [`await`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/await)
   * to wait for the magic to happen.
   *
   * @example
   ...
   *  module.exports = function(robot) {
   *    robot.on('issues.opened', async context => {
   *      const ql = await robot.getGraphQLClient();
   *    });
   *  };
   *
   * @param {number} [id] - ID of the installation, which can be extracted from
   ...
   *
   * @param {object} [headers] - A object that will be sent as headers when
   * creating the client. Use this to send additional headers or overwriting
   * exiting ones. In most cases you don't want to supply any additional headers
   * and call the function with out this parameter specified.
   *
   * @returns {Promise<ql>} - An authenticated GitHub GraphQL client
   * @private
   */
  async getGraphQLClient (id, headers = {}) {
    const token = await this.getToken(id)
  ...
  async getToken (id, log = this.log) {
    const github = new GitHubApi({
      debug: process.env.LOG_LEVEL === 'trace',
      host: process.env.GHE_HOST || 'api.github.com',
      pathPrefix: process.env.GHE_HOST ? '/api/v3' : '',
    ...
    if (id) {
      ...
      log.trace('No app ID specified. Use getToken(id)')
    }
  }
}
